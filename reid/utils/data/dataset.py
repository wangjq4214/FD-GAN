...
class Dataset:
    ...
    def load(self, num_val=0.3, verbose=True):
        ...
        self.train, self.train_query = _pluck(
            identities, train_pids, relabel=True)
        self.val, self.val_query = _pluck(identities, val_pids, relabel=True)
        self.trainval, self.trainval_query = _pluck(
            identities, trainval_pids, relabel=True)
        self.query, self.query_query = _pluck(identities, self.split['query'])
        self.gallery, self.gallery_query = _pluck(
            identities, self.split['gallery'])
